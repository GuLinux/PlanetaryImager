...
def package(args):
  ...
  for image in filtered_images:
    image.package(code_path, destination_path, args.make_jobs, cmake_defines, stderr=args.stderr, build_directory=args.build_directory, append_security_opt_unconfined=args.security_opt_unconfined)
...
parser_package.add_argument('--security-opt-unconfined', action='store_true', default=False, help='Add --security-opt seccomp:unconfined parameter to docker run')
